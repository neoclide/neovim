...
export class Window extends BaseApi {
  ...
  public getOption(name: string): Promise<VimValue> {
    if (isCocNvim) return this.request(`nvim_get_option_value`, [name, { win: this.id }])
  ...
  public setOption(name: string, value: VimValue, isNotify?: boolean): Promise<void> | void {
    if (isCocNvim) return this[isNotify ? 'notify' : 'request'](`nvim_set_option_value`, [name, value, { win: this.id }])
